...
class TestPlatformDefaults:
    ...
    def test_medium_tier_platforms(self):
        """Mattermost, Matrix, and Feishu default to 'new' tool progress."""
        from gateway.display_config import resolve_display_setting

        for plat in ("mattermost", "matrix", "feishu"):
            assert resolve_display_setting({}, plat, "tool_progress") == "new", plat

    def test_whatsapp_defaults_tool_progress_off(self):
        """WhatsApp defaults to quiet tool progress to avoid leaking raw tool names."""
        from gateway.display_config import resolve_display_setting
